"""
VERSION 3: Mixed Approach - Different rad directions for each conflict pair
"""
...
st.set_page_config(page_title="Version 3 - Mixed Approach", page_icon="🗺️", layout="wide")
...
def create_network_graph(graph_obj, path=None):
    ...
    # VERSION 3: MIXED - Alternating positive/negative rads strategically
    edge_styles = {
        ('A', 'M'): 'arc3,rad=0.45',     
        ('A', 'S'): 'arc3,rad=-0.5',     # Strong negative to avoid A-M
        ('A', 'B'): 'arc3,rad=0.25',
        ('A', 'C'): 'arc3,rad=0.55',     
        ('B', 'M'): 'arc3,rad=-0.45',    # Negative to separate from A-M
        ('B', 'S'): 'arc3,rad=0.35',     # Positive to separate from A-S
        ('B', 'F'): 'arc3,rad=-0.2',
        ('C', 'M'): 'arc3,rad=-0.6',     # Strong negative
        ('C', 'R'): 'arc3,rad=0.35',     # Positive
        ('C', 'T'): 'arc3,rad=0.45',
        ('R', 'M'): 'arc3,rad=0.55',     # Strong positive to separate from C-M
        ('R', 'F'): 'arc3,rad=0.2',
        ('T', 'F'): 'arc3,rad=-0.25',
        ('T', 'H'): 'arc3,rad=0.2',
        ('F', 'H'): 'arc3,rad=0.18',
    ...
    ax.set_title("VERSION 3: Approche Mixte (Alternance +/-)", fontsize=18, fontweight='bold', pad=20)
    ax.axis('off')
    ax.margins(0.15)
    plt.tight_layout()
    return fig
...
def main():
    st.markdown('<div class="main-header">🗺️ Version 3: Approche Mixte</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-header">Alternance stratégique des courbures positives/négatives</div>', unsafe_allow_html=True)
